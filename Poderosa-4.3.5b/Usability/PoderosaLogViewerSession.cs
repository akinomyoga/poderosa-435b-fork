 ...
 *
 * $Id: PoderosaLogViewerSession.cs,v 1.3 2011/08/06 04:46:34 kzmi Exp $
 */
using System;
...
namespace Poderosa.LogViewer {
	...
	internal class PoderosaLogViewControl : CharacterDocumentViewer, IPoderosaView, IGeneralViewCommands {
		...
		public static int DefaultWidth {
		  get {
				using(RenderProfile profile=CreateLogRenderProfile())
					return (int)(profile.Pitch.Width * PoderosaLogDocument.DefaultWidth);
		  }
		}
	...
	internal class PoderosaLogDocument : CharacterDocument, IPoderosaLogListener {
		...
		public void OnNewItem(IPoderosaLogItem item) {
			...
			while(offset < text.Length) {
				...
				GLine line = new GLine(text.Substring(offset, next-offset).ToCharArray(), new GWord(TextDecoration.Default, 0, CharGroup.Hankaku));
		...
		private void Append(GLine line) {
			if(this.Size==1 && _firstLine.Text.Length==0) {　//初回。ちょっちきたねえな
				_firstLine = line;
				_lastLine = line;
				line.ID = 0;
			}
