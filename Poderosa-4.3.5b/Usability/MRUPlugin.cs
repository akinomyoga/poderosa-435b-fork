 ...
 *
 * $Id: MRUPlugin.cs,v 1.3 2011/08/03 16:07:52 kzmi Exp $
 */
using System;
...
namespace Poderosa.Usability
{
    ...
    internal class MRUItem : IAdaptable {
        ...
        private void AssureContent() {
            if(_lateBindContent==null) return;
        ...
        ILoginDialogUISupport
    {
        ...
        public void ApplyLoginDialogInfo(ITelnetSSHLoginDialogInitializeInfo info) {
            ...
            foreach(MRUItem item in _data) {
                ...
                if(tcp!=null) info.AddHost(tcp.Destination);
                if(tcp!=null) info.AddPort(tcp.Port);
                if(ssh!=null) info.AddIdentityFile(ssh.IdentityFileName);
                if(ssh!=null) info.AddAccount(ssh.Account);
            }
        }
    ...
    internal class OpenMRUCommand : IPoderosaCommand {
        public CommandResult InternalExecute(ICommandTarget target, params IAdaptable[] args) {
            ...
            if(ssh!=null) ssh.PasswordOrPassphrase = ""; //MRUからのSSH起動はパスワード入力は外せない。オプションで省略化にしてもいいが
    ...
    internal class MRUOptions : SnapshotAwarePreferenceBase, IMRUOptions {
        ...
        public MRUOptions(IPreferenceFolder folder) : base(folder) {
        }
