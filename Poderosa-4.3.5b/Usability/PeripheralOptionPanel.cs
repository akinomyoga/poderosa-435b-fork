 ...
 *
 * $Id: PeripheralOptionPanel.cs,v 1.3 2010/12/29 12:51:20 kzmi Exp $
 */
using System;
...
namespace Poderosa.Forms
{
    internal class PeripheralOptionPanel : UserControl
	{
        ...
        //ViewSplitIndex系
        private int ToViewSplitOptionIndex(Keys key) {
            switch(key) {
                case Keys.Control:
                    return 1;
                case Keys.Shift:
        ...
        private Keys ParseViewSplitModifier(int index) {
            switch(index) {
                case 1:
                    return Keys.Control;
                case 2:
        ...
        public bool Commit(ITerminalEmulatorOptions options, ICoreServicePreference window_options) {
			...
			try {
                ...
                options.Zone0x1F = (KeyboardStyle)_zone0x1FBox.SelectedIndex;
				...
				options.RightButtonAction = (MouseButtonAction)_rightButtonAction.SelectedIndex;
                options.MiddleButtonAction = (MouseButtonAction)_middleButtonAction.SelectedIndex;

				itemname = sr.GetString("Caption.OptionDialog.MousewheelAmount");
				options.WheelAmount = Int32.Parse(_wheelAmount.Text);

                window_options.ViewSplitModifier = ParseViewSplitModifier(_viewSplitModifierBox.SelectedIndex);
        ...
        private void AdjustAutoKeySettings(object sender, EventArgs args) {
            ...
            if(_send0x7FByDel.Checked) bld.Append("Delete=0x7F");
            if(_send0x7FByBack.Checked) {
                if(bld.Length>0) bld.Append(", ");
                bld.Append("Back=0x7F");
            }

            KeyboardStyle ks = (KeyboardStyle)_zone0x1FBox.SelectedIndex;
    ...
    internal class PeripheralOptionPanelExtension : OptionPanelExtensionBase {
        ...
        public override void InitiUI(IPreferenceFolder[] values) {
            if(_panel==null) _panel = new PeripheralOptionPanel();
