...
* $Id: ConnectionOptionPanel.cs,v 1.3 2010/12/29 12:51:20 kzmi Exp $
*/
using System;
...
namespace Poderosa.Forms {
    ...
    internal class ConnectionOptionPanel : UserControl
	{
		...
		public void InitUI(IProtocolOptions options) {
            ...
            _IPv6PriorityBox.SelectedIndex = (int)options.IPVersionPriority;
		}
		public bool Commit(IProtocolOptions options) {
			...
			try {
				...
				foreach(string c in _socksNANetworksBox.Text.Split(';')) {
					if(c.Length>0 && !NetAddressUtil.IsNetworkAddress(c)) throw new FormatException(); //TODO‚±‚±‚¾‚¯ƒiƒ“ƒgƒJ‚·‚ê‚ÎNetUtil‚Íinternal‚É‚Å‚«‚é
				}
				options.SocksNANetworks = _socksNANetworksBox.Text;

                options.IPVersionPriority = (IPVersionPriority)_IPv6PriorityBox.SelectedIndex;
				return true;
			}
    ...
    internal class ConnectionOptionPanelExtension : OptionPanelExtensionBase {
        ...
        public override void InitiUI(IPreferenceFolder[] values) {
            if(_panel==null) _panel = new ConnectionOptionPanel();
