 ...
 *
 * $Id: ShortcutFile.cs,v 1.1 2010/11/19 15:41:20 kzmi Exp $
 */
using System;
...
namespace Poderosa.Sessions {
    internal class ShortcutFileContent {
        ...
        private static ShortcutFileContent ParseV4(XmlElement root) {
            ...
            if(second==null) throw new FormatException("Unknown XML Format");
            ...
            if(setting==null) throw new FormatException("TerminalSettings could not be loaded");
            ITerminalParameter param = ss.Deserialize(parameter_text) as ITerminalParameter;
            if(param==null) throw new FormatException("TerminalParameter could not be loaded");
        ...
        private static ShortcutFileContent ParseOldFormat(XmlElement root) {
            if(root.GetAttribute("type")!="tcp") throw new FormatException("Unknown File Format");
            ...
            foreach(XmlAttribute attr in root.Attributes) {
                switch(attr.Name) {
                    case "auth":
                        if(ssh!=null) ssh.AuthenticationType = ParseUtil.ParseEnum<AuthenticationType>(attr.Value, AuthenticationType.Password);
                        break;
                    case "keyfile":
                        if(ssh!=null) ssh.IdentityFileName = attr.Value;
                        break;
                    case "encoding":
                    ...
                    case "port":
                        ...
                        break;
                    case "method":
                        if(ssh!=null) ssh.Method = attr.Value=="SSH1"? SSHProtocol.SSH1 : SSHProtocol.SSH2;
                        break;
                }
            }
