 ...
 *
 * $Id: TerminalSessionPlugin.cs,v 1.2 2011/08/03 16:07:51 kzmi Exp $
 */
using System;
...
namespace Poderosa.Sessions
{
    ...
    internal class LoginDialogInitializeInfo : ITelnetSSHLoginDialogInitializeInfo {
        ...
        public LoginDialogInitializeInfo() {
            ...
            _ports = new List<int>();
            _ports.Add(22); _ports.Add(23); //これらはデフォ
        }
        ...
        public void AddHost(string value) {
            if(!_hosts.Contains(value) && value.Length>0) _hosts.Add(value);
        }

        public void AddAccount(string value) {
            if(!_accounts.Contains(value) && value.Length>0) _accounts.Add(value);
        }

        public void AddIdentityFile(string value) {
            if(!_identityFiles.Contains(value) && value.Length>0) _identityFiles.Add(value);
        }

        public void AddPort(int value) {
            if(!_ports.Contains(value)) _ports.Add(value);
        }
