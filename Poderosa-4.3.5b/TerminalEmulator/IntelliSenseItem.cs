 ...
 *
 * $Id: IntelliSenseItem.cs,v 1.1 2010/11/19 15:41:11 kzmi Exp $
 */
using System;
...
namespace Poderosa.Terminal {
    ...
    internal class IntelliSenseItemCollection : IIntelliSenseItemCollection {
        ...
        public void UpdateItem(string[] command) {
            ...
            _items.AddFirst(newitem);
            if(_items.Count>TerminalEmulatorPlugin.Instance.TerminalEmulatorOptions.ShellHistoryLimitCount) _items.RemoveLast();
        }
        ...
        public IIntelliSenseItem FindItem(string[] command) {
            ...
            while(e.MoveNext()) {
                IntelliSenseItem item = e.Current as IntelliSenseItem;
                if(item.CompareTo(newitem)==0) return item;
            }
            return null;
        }
    ...
    internal class IntelliSenseItem : IIntelliSenseItem {
        ...
        public string Format(char delimiter) {
            ...
            for(int i=_startIndex; i<_text.Length; i++) {
                if(i>_startIndex) bld.Append(delimiter);
                bld.Append(_text[i]);
            }
            return bld.ToString();
        }
        ...
        public MatchForwardResult MatchForward(string[] src) {
            if(src.Length==0) return MatchForwardResult.PartialArg;
            if(this.Length < src.Length) return MatchForwardResult.None;

            for(int i=0; i<src.Length-1; i++) { //最終項以外は、完全一致でないと
                if(src[i]!=_text[_startIndex + i]) return MatchForwardResult.None;
            }
