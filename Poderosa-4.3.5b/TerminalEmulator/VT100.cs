 ...
 *
 * $Id: VT100.cs,v 1.10 2011/04/04 14:24:02 kzmi Exp $
 */
using System;
using System.IO;
using System.Text;
	...
	internal class VT100Terminal : EscapeSequenceTerminal {
		...
		protected void ProcessSGR(string param) {
			...
			foreach(string cmd in ps) {
				...
				if (state == 1 || state == 2) {
					...
					if (code < 256) {
						dec = dec.GetCopyWithTextColor(GetRenderProfile().ESColorSet[code]);
					}
					state = 0;
				}
				else if (state == 4) {
					if (code < 256) {
						Color c = GetRenderProfile().ESColorSet[code];
						dec = dec.GetCopyWithBackColor(DrawUtil.DarkColor(c)); //背景色は暗めに
						// dec = dec.GetCopyWithBackColor(c); // 背景色を暗くしない (テスト用)
						if (((GetDocument().BackColor.ToArgb() != dec.BackColor.ToArgb()) && GetDocument().IsApplicationMode) && !_bgColorHasbeenSet) {
					...
					dec = dec.GetCopyWithTextColor(GetRenderProfile().ESColorSet[code - 30]);
				}
					...
					//これだと色を変更したとき既に画面にあるものは連動しなくなるが、そこをちゃんとするのは困難である
					dec = dec.GetCopyWithTextColor(GetRenderProfile().ESColorSet[code - 90 + 8]);
				}
				else if(code>=100 && code<=107) {
					Color c = GetRenderProfile().ESColorSet[code - 100 + 8];
					dec = dec.GetCopyWithBackColor(DrawUtil.DarkColor(c)); //背景色は暗めに
					if (((GetDocument().BackColor.ToArgb() != dec.BackColor.ToArgb()) && GetDocument().IsApplicationMode) && !_bgColorHasbeenSet) {
						GetDocument().BackColor = dec.BackColor;
						_bgColorHasbeenSet = true;
		...
		internal override int EncodeInputKey1(Keys key,out byte[] data) {
			...
			if((int)Keys.F1 <= (int)code && (int)code <= (int)Keys.F12) {
				...
				if((mods & Keys.Shift) != Keys.None) n += 10; //shiftは値を10ずらす
				char tail;
				if(n>=20)
