 ...
 *
 * $Id: IntelliSenseWindow.cs,v 1.1 2010/11/19 15:41:11 kzmi Exp $
 */
...
namespace Poderosa.Terminal {
    ...
    internal class IntelliSenseWindow : ToolStripDropDown {
        ...
        //ListBoxからのコールバック系
        public void Cancel() {
            if(_status==ComplementStatus.Exiting || _status==ComplementStatus.Hidden) return; //このExit内でClose()するとLostFocusなどを経由して再帰してくる場合がある
        ...
        public void DoChar(char ch) {
            ...
            if(ch==(char)0x08) { //BS
                if(_context.RemoveChar().Length==0) Cancel();
            }
        ...
        public void PartialComplement(char appending) {
            ...
            if(is_partial && appending=='\0') appending = _context.CurrentScheme.DefaultDelimiter;
        ...
        private int FindCandidateIndex(string input) {
            ...
            foreach(object t in _listBox.Items) {
                string x = t.ToString();
                //前方一致で
                if(x.StartsWith(input)) return i;
                i++;
            }
            return -1;
        }
        ...
        private string FormatSendBackText(string value, char delim) {
            ...
            char[] current_text = _context.Owner.PromptLine.Text;
            
            //送信と受信のはざまにいる奴をケア
            CheckCharQueue(ref current_text, ref caret_column);

            for(int x = _context.CommandStartPoint.X; x<caret_column; x++) {
                if(value.Length>corresponding_len && current_text[x]!=value[corresponding_len]) break;
                corresponding_len++;
            }
