 ...
 *
 * $Id: IntelliSense.cs,v 1.2 2011/08/27 14:13:35 kzmi Exp $
 */
using System;
...
namespace Poderosa.Terminal {
    ...
    internal class IntelliSense : IPromptProcessor {
        ...
        private void TryParseMultiLineCommand() {
            ...
            while(command_start_candidate!=null && limit>0) {
                string prompt;
                string command;
                if (_terminal.PromptRecognizer.DeterminePromptLine(command_start_candidate, current.ID, current.Text.Length, out prompt, out command)) {
                    if(command.Length>0) _context.UpdateCommandList(command);
                }
        ...
        private void PopupMain(char append_char) {
            ...
            if(!_context.IsEmpty) {
                if(_intelliSenseWindow==null) _intelliSenseWindow = new IntelliSenseWindow(); //遅延作成
    ...
    internal class IntelliSenseContext {
        ...
        public void UpdateCommandList(string command) {
            if(_scheme==null) return; //関連付けられたものがないときは何もしない
        ...
        public void BuildCandidates() {
            ...
            foreach(IntelliSenseItem item in col.Items) {
                ...
                if(_intelliSenseMode==IntelliSenseMode.ArgComplement && r==IntelliSenseItem.MatchForwardResult.PartialArg) {
                    ...
                    if(initial==null) initial = i; //Partialの一致がきた最初のものを初期値とする
            ...
            if(extensions.Length>0) {
                foreach(IIntelliSenseCandidateExtension e in extensions) e.AdjustItem(_owner.Terminal, _candidates, _currentInput);
            }
