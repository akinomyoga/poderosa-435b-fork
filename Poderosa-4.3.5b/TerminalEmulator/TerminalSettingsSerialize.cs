 ...
 *
 * $Id: TerminalSettingsSerialize.cs,v 1.1 2010/11/19 15:41:11 kzmi Exp $
 */
using System;
...
namespace Poderosa.Terminal {
    ...
    internal class TerminalSettingsSerializer : ISerializeServiceElement {
        ...
        public StructuredText Serialize(object obj) {
            ...
            storage.Set("encoding", EnumDescAttribute.For(typeof(EncodingType)).GetDescription(ts.Encoding));
            if(ts.TerminalType!=TerminalType.XTerm) storage.Set("terminal-type", ts.TerminalType.ToString());
            if(ts.LocalEcho) storage.Set("localecho", "true");
            if(ts.LineFeedRule!=LineFeedRule.Normal) storage.Set("linefeedrule", ts.LineFeedRule.ToString());
            if(ts.TransmitNL!=NewLine.CR) storage.Set("transmit-nl", ts.TransmitNL.ToString());
            if(ts.EnabledCharTriggerIntelliSense) storage.Set("char-trigger-intellisense", "true");
            if(!ts.ShellScheme.IsGeneric) storage.Set("shellscheme", ts.ShellScheme.Name);
        ...
        public object Deserialize(StructuredText node) {
            ...
            if(shellscheme.Length>0) ts.SetShellSchemeName(shellscheme);
            ts.Caption = node.Get("caption", "");
#if !UNITTEST
            //現在テストではRenderProfileは対象外
            StructuredText rp = node.FindChild(typeof(RenderProfile).FullName);
            if(rp!=null) ts.RenderProfile = _serializeService.Deserialize(rp) as RenderProfile;
#endif
            ts.EndUpdate();
            return ts;
        }
