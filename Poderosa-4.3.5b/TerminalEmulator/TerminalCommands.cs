 ...
 *
 * $Id: TerminalCommands.cs,v 1.3 2010/11/24 16:04:10 kzmi Exp $
 */
using System;
...
namespace Poderosa.Terminal {
  ...
  internal class TerminalCommand : GeneralCommandImpl {
    ...
    private static CommandResult CmdCopyWithOption(ICommandTarget target,TextFormatOption opt){
      ...
      if(t==null)return CommandResult.Ignored;
    ...
    private static CommandResult CmdCopyAsLook(ICommandTarget target) {
      string t = GetSelectedText(target, TextFormatOption.AsLook|TextFormatOption.TrimEol);
      if(t==null) return CommandResult.Ignored;
    ...
    private static string GetSelectedText(ICommandTarget target, TextFormatOption opt) {
      ...
      if(s.IsEmpty || !c.EnabledEx) return null;
    ...
    private static CommandResult CmdCopyToFile(ICommandTarget target) {
      ...
      if(text==null) return CommandResult.Ignored;
    ...
    private static CommandResult CmdPasteFromFile(ICommandTarget target) {
      ...
      if(dlg.ShowDialog()==DialogResult.OK) {
        ...
        try {
          ...
          if(host==null) return CommandResult.Failed;
    ...
    private static CommandResult CmdSelectAll(ICommandTarget target) {
      ...
      if(tc==null) return CommandResult.Ignored;
    ...
    private static CommandResult CmdClearBuffer(ICommandTarget target) {
      ...
      if(doc==null) return CommandResult.Ignored;
    ...
    private static CommandResult CmdNewLine(ICommandTarget target, NewLine nl) {
      ...
      if(ts==null) return CommandResult.Failed;
  ...
  internal class StandardTerminalMenuItem : PoderosaMenuItemImpl {
    public StandardTerminalMenuItem(string textID, string commandID) : base(commandID, GEnv.Strings, textID) {
    }
    public StandardTerminalMenuItem(string textID, string commandID, CheckedDelegate cd) : base(commandID, GEnv.Strings, textID)  {
      _checked = cd;
    }
  }
    ...
    private class EncodingGroup : StandardTerminalEmulatorMenuGroup {
      public override IPoderosaMenu[] ChildMenus {
        get {
          ...
          EnumDescAttribute descAttr = EnumDescAttribute.For(typeof(EncodingType));
          foreach (EncodingType enc in Enum.GetValues(typeof(EncodingType))) {
            EncodingType encodingType = enc;
            string desc = descAttr.GetRawDescription(enc);
            menus.Add(
              new StandardTerminalMenuItem(desc,
                "org.poderosa.terminalemulator.encoding." + enc.ToString(),
