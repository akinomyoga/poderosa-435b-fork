 ...
 *
 * $Id: SerialSettings.cs,v 1.3 2011/06/19 07:14:02 kzmi Exp $
 */
using System;
...
namespace Poderosa.SerialPort {
    ...
    internal class SerialTerminalParamSerializer : ISerializeServiceElement {
        ...
        public StructuredText Serialize(object obj) {
            ...
            if(tp.TerminalType!="vt100") node.Set("TerminalType", tp.TerminalType);
            if(tp.AutoExecMacroPath!=null) node.Set("autoexec-macro", tp.AutoExecMacroPath);
            return node;
        }
    ...
    internal class SerialTerminalSettingsSerializer : ISerializeServiceElement {
        ...
        public StructuredText Serialize(object obj) {
            ...
            if(ts.ByteSize!=8) node.Set("byte-size", ts.ByteSize.ToString());
            if(ts.Parity!=Parity.NOPARITY) node.Set("parity", EnumDescAttribute.For(typeof(Parity)).GetName(ts.Parity));
            if(ts.StopBits!=StopBits.ONESTOPBIT) node.Set("stop-bits", EnumDescAttribute.For(typeof(StopBits)).GetName(ts.StopBits));
            if(ts.FlowControl!=FlowControl.None) node.Set("flow-control", EnumDescAttribute.For(typeof(FlowControl)).GetName(ts.FlowControl));
            if(ts.TransmitDelayPerChar!=0) node.Set("delay-per-char", ts.TransmitDelayPerChar.ToString());
            if(ts.TransmitDelayPerLine!=0) node.Set("delay-per-line", ts.TransmitDelayPerLine.ToString());
