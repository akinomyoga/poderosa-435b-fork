 ...
 *
 * $Id: MainWindow.cs,v 1.4 2011/08/07 02:25:24 kzmi Exp $
 */
using System;
...
namespace Poderosa.Forms
{
	...
	internal class MainWindow : PoderosaForm, IPoderosaMainWindow {
		...
		protected override void OnClosing(System.ComponentModel.CancelEventArgs e) {
			base.OnClosing(e);
			try {
				if(SessionManagerPlugin.Instance==null) return; //単体テストではSessionなしで起動することもありだ
	...
	internal class WindowCaptionManager : IActiveDocumentChangeListener {
		readonly EventHandler<EventArgs> deleg_odocument_OnCaptionChanged;
		readonly System.Collections.Generic.Dictionary<IPoderosaMainWindow,IPoderosaDocument> dict
			=new System.Collections.Generic.Dictionary<IPoderosaMainWindow,IPoderosaDocument>();
		readonly System.Collections.Generic.Dictionary<IPoderosaDocument,IPoderosaMainWindow> dict2
			=new System.Collections.Generic.Dictionary<IPoderosaDocument,IPoderosaMainWindow>();

		public WindowCaptionManager(){
			this.deleg_odocument_OnCaptionChanged=new EventHandler<EventArgs>(odocument_OnCaptionChanged);
		}
		...
		void SetDocument(IPoderosaMainWindow window,IPoderosaDocument document){
			IPoderosaDocument odocument;
			if(dict.TryGetValue(window,out odocument)){
				odocument.OnCaptionChanged-=deleg_odocument_OnCaptionChanged;
				dict2.Remove(odocument);
				if(document==null)
					dict.Remove(window);
			}

			if(document!=null){
				dict[window]=document;
				dict2[document]=window;
				document.OnCaptionChanged+=deleg_odocument_OnCaptionChanged;
			}
		}
