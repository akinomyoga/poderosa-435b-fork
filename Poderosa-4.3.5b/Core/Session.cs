 ...
 *
 * $Id: Session.cs,v 1.1 2010/11/19 15:40:39 kzmi Exp $
 */
using System;
...
namespace Poderosa.Sessions
{
        ...
        ISessionManagerForViewSplitter
    {
        ...
        public PrepareCloseResult CloseMultipleDocuments(ClosingContext context, IPoderosaDocument[] documents) {
            ...
            foreach(SessionHost sh in sessions) sh.CMP_ClosingDocumentCount = 0; //カウントリセット
            ...
            foreach(SessionHost sh in sessions) {
                if(sh.CMP_ClosingDocumentCount==0) continue; //影響なし
        ...
        public void ActivateDocument(IPoderosaDocument document, ActivateReason reason) {
            ...
            try {
                ...
                if(dh.CurrentView!=null) { //既に見えている場合
                    if(reason!=ActivateReason.ViewGotFocus) SetFocusToView(dh.CurrentView); //ユーザのフォーカス指定だった場合はそれに任せる
                }
                else { //見えてはいなかった場合
