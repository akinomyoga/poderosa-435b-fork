 ...
 *
 * $Id: RenderProfileSerialize.cs,v 1.4 2010/11/24 16:04:10 kzmi Exp $
 */
using System;
...
namespace Poderosa.View {
    internal class RenderProfileSerializer : ISerializeServiceElement {
        ...
        public StructuredText Serialize(object obj) {
            ...
            if(prof.UseClearType) storage.Set("clear-type", "true");
            if(!prof.EnableBoldStyle) storage.Set("enable-bold-style", "false");
            if(prof.ForceBoldStyle) storage.Set("force-bold-style", "true");
            ...
            if(!prof.ESColorSet.IsDefault) storage.Set("escape-sequence-color", prof.ESColorSet.Format());
            return storage;
        }
        ...
        public object Deserialize(StructuredText storage) {
            ...
            if(escolor!=null) prof.ESColorSet.Load(escolor);
