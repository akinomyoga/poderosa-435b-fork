 ...
 *
 * $Id: CharacterDocument.cs,v 1.7 2011/09/01 20:34:58 kzmi Exp $
 */
using System;
...
namespace Poderosa.Document
{
	...
	public class CharacterDocument : IPoderosaDocument, IPoderosaContextMenuPoint {
		...
		protected Color _bgColor = Color.Empty;
		...
		protected GLine FindLineOrNullClipTop(int index) {
			if (index < _firstLine.ID) index = _firstLine.ID;
			else if (index > _lastLine.ID) return null;
		...
		public virtual string Caption {
			...
			set{
				...
				if(this.OnCaptionChanged!=null)
					this.OnCaptionChanged(this,new System.EventArgs());
			}
		}
		public event System.EventHandler<System.EventArgs> OnCaptionChanged;
		#endregion
	}
	...
	public class InvalidatedRegion {
		...
		public void InvalidateLine(int id) {
			lock (this) {
				_empty = false;
				if (_lineIDStart == NOT_SET || _lineIDStart > id) _lineIDStart = id;
				if (_lineIDEnd == NOT_SET || _lineIDEnd < id) _lineIDEnd = id;
			}
		}
		public void Reset() {
			lock (this) {
