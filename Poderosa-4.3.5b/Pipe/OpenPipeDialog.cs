 ...
 *
 * $Id: OpenPipeDialog.cs,v 1.1 2011/08/04 15:28:58 kzmi Exp $
 */
using System;
...
namespace Poderosa.Pipe {
    ...
    internal partial class OpenPipeDialog : Form {
        ...
        public void ApplyParams(PipeTerminalParameter param, PipeTerminalSettings settings) {
            ...
            _comboBoxLogType.SelectedIndex = (int)LogType.None;
            _textBoxLogFile.Enabled = false;
            _buttonBrowseLogFile.Enabled = false;

            _comboBoxEncoding.SelectedIndex = (int)settings.Encoding;
            _comboBoxNewLine.SelectedIndex = (int)settings.TransmitNL;
            _comboBoxLocalEcho.SelectedIndex = settings.LocalEcho ? 1 : 0;
            _comboBoxTerminalType.SelectedIndex = (int)settings.TerminalType;
        ...
        private void SetupControls() {
            ...
            _comboBoxLogType.Items.AddRange(EnumDescAttribute.For(typeof(LogType)).DescriptionCollection());
            _comboBoxEncoding.Items.AddRange(EnumDescAttribute.For(typeof(EncodingType)).DescriptionCollection());
            _comboBoxLocalEcho.Items.AddRange(new object[] {
			   res.GetString("Common.DoNot"),
			   res.GetString("Common.Do")
			});
            _comboBoxNewLine.Items.AddRange(EnumDescAttribute.For(typeof(NewLine)).DescriptionCollection());
            _comboBoxTerminalType.Items.AddRange(EnumDescAttribute.For(typeof(TerminalType)).DescriptionCollection());
        }
        ...
        private bool ValidateParams(out PipeTerminalParameter param, out PipeTerminalSettings settings) {
            ...
            try {
                ...
                TerminalType terminalType = (TerminalType)_comboBoxTerminalType.SelectedIndex;
                paramTmp.SetTerminalName(terminalType.ToString().ToLowerInvariant());

                LogType logType = (LogType)EnumDescAttribute.For(typeof(LogType)).FromDescription(_comboBoxLogType.Text, LogType.None);
                ...
                settingsTmp.Encoding = (EncodingType)_comboBoxEncoding.SelectedIndex;
                settingsTmp.LocalEcho = _comboBoxLocalEcho.SelectedIndex == 1;
                settingsTmp.TransmitNL = (NewLine)_comboBoxNewLine.SelectedIndex;
        ...
        private void _comboBoxLogType_SelectedIndexChanged(object sender, EventArgs e) {
            bool enableLogFile = (_comboBoxLogType.SelectedIndex != (int)LogType.None);
