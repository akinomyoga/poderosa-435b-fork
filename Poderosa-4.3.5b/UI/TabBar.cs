 ...
 *
 * $Id: TabBar.cs,v 1.6 2011/08/15 17:12:22 kzmi Exp $
 */
using System;
...
namespace Poderosa.Forms {
	...
	public class TabBar : UserControl {
		...
		private AdjustEachWidthResult AdjustEachWidth(int sufficient_width, int actual_width) {
			...
			if(to_be_shrinked <= 0) return AdjustEachWidthResult.Sufficient; //何もしなくて十分サイズあり
			...
			while(to_be_shrinked > 0) {
				...
				if(order==_sortedButtons.Count && to_be_shrinked>0) return AdjustEachWidthResult.Overflow; //最後まで縮小してダメならあきらめる
			}
	...
	internal class TabBarButton : UserControl {
		...
		internal int CalcSufficientWidth(Graphics g) {
			...
			_sufficientWidth =
				...
				+ 2 /* border lines */;
			if(_selected) _sufficientWidth += 2; //選択時は広めがよさそう
			return _sufficientWidth;
		}
        ...
        public int IndexOf(TabKey key) {
            ...
            foreach(TabBar bar in _bars) {
                foreach(TabBarButton button in bar.Buttons) {
                    if(button.TabKey==key) return count + button.Index;
                }
                count += bar.TabCount;
            }
            return -1;
